...
public interface FlashSaleItemRepository extends JpaRepository<FlashSaleItem, UUID> {
    ...
    @Query("SELECT COUNT(u.id) FROM FlashSaleItem u WHERE u.createdAt <= :snapshotTime")
    ...
    @Query("SELECT u.id FROM FlashSaleItem u WHERE u.createdAt <= :snapshotTime ORDER BY u.createdAt DESC, u.id DESC")
    ...
    @Query("SELECT u FROM FlashSaleItem u WHERE u.id IN :ids ORDER BY u.createdAt DESC, u.id DESC")
    List<FlashSaleItem> fetchFullDataByIds(@Param("ids") List<UUID> ids);
}
