...
public interface InventoryLedgerRepository extends JpaRepository<InventoryLedger, UUID> {
    ...
    @Query("SELECT COUNT(u.id) FROM InventoryLedger u WHERE u.createdAt <= :snapshotTime")
    ...
    @Query("SELECT u.id FROM InventoryLedger u WHERE u.createdAt <= :snapshotTime ORDER BY u.createdAt DESC, u.id DESC")
    ...
    @Query("SELECT u FROM InventoryLedger u WHERE u.id IN :ids ORDER BY u.createdAt DESC, u.id DESC")
    List<InventoryLedger> fetchFullDataByIds(@Param("ids") List<UUID> ids);
}
