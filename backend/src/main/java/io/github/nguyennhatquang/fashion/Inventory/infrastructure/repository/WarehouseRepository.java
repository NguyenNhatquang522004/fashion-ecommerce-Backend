...
public interface WarehouseRepository extends JpaRepository<Warehouse, UUID> {
    ...
    @Query("SELECT COUNT(u.id) FROM Warehouse u WHERE u.createdAt <= :snapshotTime")
    ...
    @Query("SELECT u.id FROM Warehouse u WHERE u.createdAt <= :snapshotTime ORDER BY u.createdAt DESC, u.id DESC")
    ...
    @Query("SELECT u FROM Warehouse u WHERE u.id IN :ids ORDER BY u.createdAt DESC, u.id DESC")
    List<Warehouse> fetchFullDataByIds(@Param("ids") List<UUID> ids);
}
